...
const CameraOptions = ["Image", "Video"];
export default class CreatePost extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      isBusy: false,
      videoData: false,
      itempost: this.props.route.params.itempost,
      height: 0,
      itemShowId: "",
      media_type: "",
      postId: "",
  ...
  componentDidMount() {
    ...
    console.log("itemPostttttt>>>>>", this.state.itempost);
    if (this.state.itempost != null) {
      this.setState({
        comment: this.state.itempost.content,
        postId: this.state.itempost.id,
        itemShowId: this.state.itempost.id,
        fileUri: this.state.itempost.media,
        media_type: this.state.itempost.media_type,
        communityEditId: this.state.itempost.community_id
      });
    } else {
...
{
  this.state.communityEditId != '' ?   this.postingAsAPI() : null
}
  ...
  createPost() {
    ...
    console.log('community_id>>>>>>>>>',this.state.communityId)
    if(this.state.communityEditId != null){
      data.append("community_id", this.state.communityEditId);
    }
   ...
   if (this.state.fileUri != null) {
      ...
      {
        this.state.communityEditId != null ?
        data.append("post[deleted_media]", true)
        :
        null
      }
    ...
    {
      this.state.communityEditId != null ?
      // https://api.lapared.com/api/v1/communities/94/posts/394
      api.put("/posts/"+ this.state.itemShowId + "", data, {}).then((res) =>
      this.afterPost(res).catch((err) => {
                      ...
                      <ScrollView style={{ flex: 1 }}>
                        <TextInput
                          ...
                          style={[
                            ...
                            styles.feedTextInput,
                            ,
                            {
                              height: Math.max(height * 0.1, this.state.height),
                            },
                          ]}
                          numberOfLines={5}
                        ...
                        <View style={{ height: "100%", width: "100%" }}>
                          {this.state.itemShowId == "" ? (
                            <Fragment>
                              ...
                              {fileUri != null ? (
                                <TouchableOpacity
                                  onPress={() => this.setState({fileUri: null })}
                                  style={{
                                    alignSelf: "flex-end",
                                    position: "absolute",
                                    top: -10,
          ...
          >
            ...
            >
              <Image
                resizeMode="contain"
                source={
                  videoRecording ? images.shotButtonRed : images.shotButton
                }
